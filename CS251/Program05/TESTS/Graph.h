...
class graph {
  ...
  public:
    ...
    struct vertex_label {
      ...
      vertex_label( double _dist=0.0, int _pred=-1, char _state='?',
          int _npaths=0) 
        : dist { _dist }, pred { _pred }, state { _state}, npaths { 0 }, opaths { 0 }
      { }
      ...
      while(!q.empty()) {
        ...
        for(edge &e : vertices[u].outgoing) {
          ...
          if(report[v].state == UNDISCOVERED) {
            ...
            report[v].npaths = report[u].npaths;    //updating npaths
            report[v].pred = u;
            report[v].state = DISCOVERED;
            // enqueue newly discovered vertex
            q.push(v);
          }
          //updating npaths if already discovered, and from u to v, v's distance is equal to 
          //the current shortest path's distance)
  ...
  public:
    ...
    bool extract_path(const vector<vertex_label> & rpt, int dest, vector<int> & path) {
      ...
      //run this loop to go through the predecessors from source
      while (rpt[dest].pred != dest) {  
        //if there is no connection from source, clear the path and return
        if (rpt[dest].pred == -1) {
          path.clear();
          return false;
        }
        ...
        //add in the source vertex
        path.push_back(dest);
        //reverse the path to be in correct order from dest to source
    ...
    void _dag(int i, vector<vertex_label> & rpt) {
      int bestWeight = 0;
      int testWeight = 0;

      if (vertices[i].incoming.size() < 1) { //this is a start vertex of the graph
        rpt[i].pred = i;
        rpt[i].dist = 0;
        ...
        for (int x = 0; x < vertices[i].incoming.size(); x++) {
          //properly align the incoming vertex to what we need to update
          string name = id2name(vertices[i].incoming[x].vertex_id);
          int rptNum = name2id(name);

          //if we haven't discovered it yet, we want to traverse the node to find it's max distance
          //this lets us not look recursively through nodes that we have already confirmed
          if (rpt[rptNum].state != DISCOVERED) {
            _dag(vertices[i].incoming[x].vertex_id,rpt);
          }
          //we will do a max distance check, regardless of if it is new or not. 
          if (rpt[i].dist < (vertices[i].incoming[x].weight + rpt[rptNum].dist)) {
            bestWeight = (vertices[i].incoming[x].weight + rpt[rptNum].dist);
            rpt[i].pred = vertices[i].incoming[x].vertex_id;
          }

          rpt[i].dist = bestWeight;
          rpt[i].state = DISCOVERED;
    ...
    bool dag_critical_paths(vector<vertex_label> & rpt) {
      ...
      for (int k =0; k < num_nodes(); k++) {
        if (rpt[k].state == UNDISCOVERED) {
          rpt[k].dist = 0;                  //reset distance to 0 from -1 if undiscovered
        }
        if (vertices[k].outgoing.size() < 1) {  //run dag longest path for every end vertex
            _dag(k, rpt);
          }
        }
      return true;
     ...
     *
     */
    //INCOMING VERTEX HELPER
    int _inumpaths(int i, vector<vertex_label> & rpt) {
      if (vertices[i].incoming.size() < 1) { //this is a start vertex of the graph
        // cout << vertices[i].name <<  " is an input vertex.\n"; 
        rpt[i].state = DISCOVERED;
        rpt[i].npaths = 1;
        return rpt[i].npaths;
      }

      else {  //enumerate incoming
        for (int x = 0; x < vertices[i].incoming.size(); x++) {
          string name = id2name(vertices[i].incoming[x].vertex_id);
          int rptNum = name2id(name);

          if (rpt[rptNum].state != DISCOVERED) { //if we haven't calculated a vertex yet
            _inumpaths(vertices[i].incoming[x].vertex_id, rpt);            
          }
          //add the previous paths together
          rpt[i].state = DISCOVERED;
          rpt[i].npaths += rpt[rptNum].npaths;
        }
      }
      return rpt[i].npaths;
    }

    //OUTGOING VERTEX HELPER
    int _onumpaths(int i, vector<vertex_label> & rpt) {
      if (vertices[i].outgoing.size() < 1) { //this is an end vertex of the graph
        rpt[i].opaths = 1;             
        rpt[i].state = DISCOVERED;
        return rpt[i].opaths;
      }

      else {  //enumerate outgoing
        for (int x = 0; x < vertices[i].outgoing.size(); x++) {
          string name = id2name(vertices[i].outgoing[x].vertex_id);
          int rptNum = name2id(name);

          if (rpt[rptNum].state != DISCOVERED) { //if we haven't calculated a vertex yet
            rpt[i].opaths += _onumpaths(vertices[i].outgoing[x].vertex_id, rpt);        
          }

          else {
            rpt[i].opaths += rpt[rptNum].opaths;
          }
    ...
    //OUTGOING HELPER WRAPPER
    int opathwrapper(int i, vector<vertex_label> &rpt) {
      for (int j =0; j < num_nodes(); j++) {
        rpt[j].opaths = 0;
        rpt[j].state = UNDISCOVERED;
      }
      return _onumpaths(i,rpt);
    }
    ...
    bool dag_num_paths(vector<vertex_label> & rpt) {
      ...
      for (int j = 0; j < num_nodes(); j++) {
        rpt[j].npaths = 0;
        _inumpaths(j, rpt); //get the incoming pathways
      }
      for (int i = 0; i < num_nodes(); i++ ) {
        rpt[i].npaths *= opathwrapper(i,rpt); //multiply incoming pathways with outgoing           
      }   
    ...
    bool valid_topo_order(const vector<int> & order) {
      ...
      if(has_cycle())
        ...
        for (int i = 0; i < num_nodes(); i++) {
          //if it is a start vertex, no need to check the incoming
          if (vertices[order[i]].incoming.size() < 1) {
            vertices[order[i]].state = DISCOVERED;
          }

          //if it is not a start vertex, we need to check the incoming list
          else {
            for (int j = 0; j < vertices[order[i]].incoming.size(); j++ ) {
              incID = vertices[order[i]].incoming[j].vertex_id;
              name = id2name(vertices[order[i]].incoming[j].vertex_id);

              //if we find that it hasn't been discovered, then it isn't the right sorting
              if (vertices[incID].state != DISCOVERED) {
                return false;
              }
              //set to discovered if we validate all incoming have been discovered
              vertices[order[i]].state = DISCOVERED;
            }
          }
        }
    ...
    void _enum_paths(int target, int currvertex, vector<string> &paths, string pathway){
      string currpathway;
      string next;
      // cout << "looking at vertex: " << vertices[target].name << endl;

      if (vertices[currvertex].id == target) {
        // this is the destination vertex, so we finish by pushing into paths
        paths.push_back(pathway);
        return;
      }

      if (vertices[currvertex].incoming.size() < 1) {
        //this is a starting vertex
        next =  id2name(vertices[currvertex].id);
        pathway += next;
      }      

      if (vertices[currvertex].outgoing.size() >= 1) {
        for ( int i=0; i < vertices[currvertex].outgoing.size(); i++) {
          next = id2name(vertices[currvertex].outgoing[i].vertex_id);
          currpathway = pathway;
          pathway += " " + next;
          _enum_paths(target, name2id(next), paths, pathway);
          pathway = currpathway;
        }        
      }
      return;
    ...
    bool enum_paths(int target, vector<string> &paths) {
      ...
      for (int i=0; i<num_nodes(); i++) {
        if (vertices[i].incoming.size() < 1) {
          cout << vertices[i].name << " is a start vertex. starting...\n\n";
          _enum_paths(target, vertices[i].id, paths, pathway);       //this will recursively provide strings
          cout << endl;
        }
      }         
