...
//puts the values of a bst into a vector in-order
    ...
    int num_range(const T & min, const T & max) {
      ...
      // consider if all on left
      // consider if all on right  
      // consider if in between and includes root node
      //   you can do a geq on min and leq on max
    }
