...
/** @file t7_merge-C.cpp
        ...
        functional tests of the merge function:  
			[]  and  [5 7 7 10 12] are
		merged.  Two separate test cases
		then check the two lists after call.
...
int main(int argc, char *argv[]){
  ...
  START("~List::merge_with COMPLETE test Suite (C) - 6.667 pts"); 
  {
	int i;
	...
	cksum_before = lsts[0]->checksum() ^ lsts[1]->checksum();;
	  ...
	  TEST_RET(cksum_and_eq2array(lsts[0], a, 5, cksum_before), 
			"Test merge [] & [5 7 7 10 12] (including 'no-new-nodes' rule",
			1, PtsPer);
