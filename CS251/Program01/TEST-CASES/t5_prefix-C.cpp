...
/** @file t5_prefix-C.cpp

	two functional tests of the prefix function: a length-3
	prefix of a list of length six is requested, then both resulting
	lists are tested (as individual tests).
...
int main(int argc, char *argv[]){
  ...
  START("~List::prefix COMPLETE test Suite (4 pts)"); 
  {
	int i;
	  ...
	  TEST_RET(checksum_tst_2lists(lsts[0], lsts[1], cksum_before),
			"length-3 prefix test: 'no-new-nodes' test",
			1, 1.5);
	  ...
	  TEST_RET_MESSAGE(eq2array(lsts[0], right3, 3), 
			"length-3 prefix test: RHS correct?",
			1, 1.25);
