...
class GridWorld {
    ...
    class doubleLinkedList {
      ...
      void pop_out(Person * &leavingP) {          
        if (front == back && back == nullptr) {           //empty list
          ...
          leavingP->next->previous = nullptr;
      ...
      void printList() {
        ...
        while (temp != nullptr) {
          cout << temp->pID << " ";
          // if (temp->next != nullptr) {cout << " Next in line: " << temp->next->pID << endl;}
          // if (temp->previous != nullptr) {cout << " Before in line: " << temp->previous->pID << endl;}
          temp = temp->next;
        }
        cout << endl;
      }
  ...
  public:
    ...
    void getBeforeAndAfter(int ID) {
      cout << "Person ID: " << personVector[ID].toDistrictList->pID << endl;
      cout << "Before: " << personVector[ID].toDistrictList->previous->pID << endl;
      cout << "After: " << personVector[ID].toDistrictList->next->pID << endl;
    }
    /*--------------------------------Prog02 functions--------------------------------*/ 
    /*
     * function: birth
